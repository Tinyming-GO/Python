...
import requests
import pymongo  # sudo python -m pip install pymongo  安装模块方法
import datetime
...
def func(page):
    ...
    items = re.findall(r'"type":"(.*?)","codeType".*?"contentHtml":"(.*?)","data".*?"categorySet":"(.*?)","hasMore"',
                       content)  # 正则匹配
    if len(items) == 0:
        print
        "The End Page:", page
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        content = Spider(url, data)
        items = re.findall(
            r'"type":"(.*?)","codeType".*?"contentHtml":"(.*?)","data".*?"categorySet":"(.*?)","hasMore"',
            content)  # 正则匹配
        if len(items) == 0:
            print
            "The End Page:", page
