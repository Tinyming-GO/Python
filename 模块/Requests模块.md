...
```bash
>>> r.encoding   # 显示服务器返回的内容的编码
'utf-8'
>>> r.encoding = 'ISO-8859-1'  # 将内容转换成新的编码
```
...
>>> i = Image.open(BytesIO(r.content))  # BytesIO() 从内存中读取二进制数据  Image.open() 从文件中读取一张图片,返回一个Image对象
```
...
>>> r = requests.post(url, data=json.dumps(payload))  # json.dumps() 将 Python 对象编码成 JSON 字符串。相反，json.loads()将已编码的 JSON 字符串解码为 Python 对象
```
